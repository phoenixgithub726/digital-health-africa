...
const Chat = ({userId, otherUserId, otherUserName, openChatField}) => {
    ...
    db
      .onSnapshot(snapshot => {
        ...
        snapshot.forEach(doc => {
          ...
          message_buf.push({id: doc.data().id, isOwnMessage: doc.data().from === userId, body: doc.data().body, date: doc.data().date.seconds})
        })
  ...
  const handleSendMessage = () => {
    if (pendingMessage === '') {
      ...
      db
        .add({
          from: userId, to: otherUserId, date: firebase.firestore.Timestamp.fromDate(new Date()), body: pendingMessage
        })
        .then((docRef) => {
          db
            .doc(docRef.id)
    ...
    return(
        <Grid 
            container className={patientstyles['chat']} 
            style={{visibility: openChatField? 'visible' : 'hidden'}}
        >
            <Grid item xs={12} className={patientstyles['Chat__titlebar__details']}>
                <span>{!otherUserId
                    ? 'Loading...'
                    : otherUserName}
                </span>
            ...
            <Grid container item className={patientstyles['Chat__messages']} >
                {messages.map(m => (
                    <Message key={m.id} {...m} />
                ))}
            </Grid>
            <Grid container item className={patientstyles['Chat__compose']}>
                <input
                    className={patientstyles['Chat__compose__input']}
                    type="text"
                ...
                />
                <button className={patientstyles['Chat__compose__button']} onClick={handleSendMessage}>
                    Send
                </button>
            </Grid>
        </Grid>      
...
function Message({ isOwnMessage, isLatestMessage, date, body }) {
  return (
    <Grid
        container
        direction="row"
